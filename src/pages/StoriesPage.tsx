...
function ChapterMedia({ story, onPhoto }: { story: typeof STORIES[number]; onPhoto: (photos: Photo[], index: number) => void }) {
  ...
  return <div className="chapter-media">
    {story.film && <div className="chapter-film">
      <div className="media-heading"><h3>{story.id === 'manufacturing' ? 'The path to production' : story.id === 'antenna' ? 'Four steps toward a thinner antenna' : 'A moment in the making'}</h3><span>{story.id === 'manufacturing' ? '30-second process animation' : '30-second chapter film'}</span></div>
      ...
      <p className="film-editorial-note">{story.id === 'manufacturing' ? 'Original animation of our planned manufacturing workflow. Simplified shapes and test displays are illustrative; this is not factory footage or measured test data.' : story.id === 'antenna' ? 'Development explanation with simplified antenna geometry, followed by real copper-foil tuning footage. Illustrations are not measured VSWR results.' : `Edited from our workshop footage${['enclosure', 'pcb', 'antenna'].includes(story.id) ? ' and selected images' : ''}.`} Original instrumental soundtrack.</p>
    </div>}
