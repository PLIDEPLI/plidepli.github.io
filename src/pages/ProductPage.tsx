...
const GALLERY = [
  ...
  ['connected', 'Cable connections', 'Close view of the Plidépli housing and connected cables'],
]
const MODES = [
  { title: 'Internal', heading: 'Start with what’s built in.', text: 'Use the indoor antenna integrated into the booster. It keeps the room-side setup together and removes the separate indoor antenna cable run.', image: 'built-in-antenna-switch.png', width: 653, height: 384 },
  { title: 'External', heading: 'Place coverage independently.', text: 'Connect a separate approved indoor antenna when your layout calls for a different antenna position. The booster and indoor antenna can then be placed separately.', image: 'mode-external.webp?v=full-frame-1', width: 1600, height: 900 },
  { title: 'Dual', heading: 'Work with both indoor antennas.', text: 'Use the built-in antenna together with a separate approved indoor antenna. Positioning and antenna separation still determine how the system works in your space.', image: 'mode-dual.webp?v=full-frame-1', width: 1600, height: 900 },
]
...
export default function ProductPage() {
  ...
  const photos = GALLERY.map(([file, , alt], index) => ({ src: `${BASE}product/${file}.webp`, alt: `${alt}. Product rendering.`, caption: `${GALLERY[index][1]}. Product rendering.` }))
  return <>
    <section className="product-page-hero" id="top"><div className="container">
      ...
      </div><div className="product-detail-copy"><p className="eyebrow">Plidépli cellular signal booster</p><h1 className="phrase-heading"><span>A stronger connection.</span><span>A simpler setup.</span></h1><p className="product-lede">Bring your existing outdoor cell signal inside, with the indoor antenna built right into the booster.</p><div className="product-price"><strong>{CAMPAIGN_PRICE}</strong><span>Target campaign price</span></div><a className="button button-dark" href="#cta">Notify me at launch<Icon name="arrow" /></a><ul className="product-points"><li><Icon name="check" />Desk-friendly design</li><li><Icon name="check" />Flexible internal & external antenna switching</li><li><Icon name="check" />Five supported cellular bands</li><li><Icon name="check" />FCC certified</li></ul></div></div>
    </div></section>
    ...
    <section className="section kit-section" id="kit"><div className="container"><div className="section-heading"><div><h2>A system that<br />works together.</h2></div></div><div className="component-grid">{[
      ...
      ['coaxial-cable', '50 ft outdoor cable', 'A 50 ft (15 m) coaxial cable connects the outdoor antenna to the booster.'],
    ...
    ].map(([file, title, description]) => <article key={file}><div><img src={file === 'coaxial-cable' ? `${BASE}brochure/coaxial-cable.png` : `${BASE}product/${file}.webp`} alt={`${title} product rendering`} width="1254" height="1254" loading="lazy" /></div><h3>{title}</h3><p>{description}</p></article>)}</div><aside className="optional-accessories"><p className="eyebrow">Optional accessories</p><h3>More options for your installation.</h3><div className="accessory-grid">{[['whip-antenna', 'Whip antenna'], ['panel-antenna', 'Panel antenna'], ['ceiling-antenna', 'Ceiling dome antenna'], ['low-profile-antenna', 'Low-profile ceiling antenna'], ['coaxial-cable', 'Indoor antenna cable'], ['omni-antenna', 'Outdoor omni antenna'], ['outdoor-mast', 'Outdoor mounting pole']].map(([file, label]) => <figure key={file}><img className={file === 'omni-antenna' ? 'rotate-180' : undefined} src={`${BASE}brochure/${file}.png`} alt={label} loading="lazy" width="160" height="160" /><figcaption>{label}</figcaption></figure>)}</div><p>Optional accessories are sold separately. Use the approved antenna and cable combination for your mode.</p></aside></div></section>
    <section className="section fit-section" id="fit"><div className="container"><div className="section-heading"><div><h2>A good fit starts<br />outside your home.</h2></div><p>For fixed US homes and cabins, start with these three checks. Your outdoor signal, walls, and antenna placement determine indoor coverage.</p></div><div className="fit-grid">{[
      ...
      ['A suitable installation', 'You can mount the outdoor antenna, route its cable, and maintain the antenna separation shown in the installation guide.'],
