...
for chapter in SCENES:
 ...
 film=OUT/chapter/'film.mp4';before=stream_hash(film,'v');score=OUT/chapter/'score.m4a';cfg=compose(chapter,score,WORK)
 temp=WORK/f'{chapter}-scored.mp4'
 subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-i',str(film),'-i',str(score),'-map','0:v:0','-map','1:a:0','-c','copy','-t','30','-movflags','+faststart','-map_metadata','-1',str(temp)],check=True)
 ...
 report[chapter]={**cfg,'file':str(score.relative_to(ROOT)),'duration':30,'video_unchanged':True,'audio_sha256':stream_hash(film,'a')}
