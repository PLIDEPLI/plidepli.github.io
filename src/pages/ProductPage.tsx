...
const GALLERY = [
  ['kit', 'The complete kit', 'PLIDEPLI kit with booster, outdoor antenna, cable, power adapter and packaging'],
  ['booster', 'The booster', 'White PLIDEPLI booster from an angled view'],
  ['standing', 'Standing view', 'PLIDEPLI standing upright with connected antenna cables'],
  ['connected', 'Connections', 'Close view of the PLIDEPLI housing and connected cables'],
  ['board-inside', 'Inside the housing', 'PLIDEPLI circuit board and shielding inside the open housing'],
]
const MODES = [
  { title: 'Internal', heading: 'Start with what’s built in.', text: 'Use the indoor antenna integrated into the booster. It keeps the room-side setup together and removes the separate indoor antenna cable run.', image: 'antenna-inside', note: 'A good starting point when the booster can be placed near the area you want to serve.' },
  { title: 'External', heading: 'Place coverage independently.', text: 'Connect a separate approved indoor antenna when your layout calls for a different antenna position. The booster and indoor antenna can then be placed separately.', image: 'connected', note: 'An external indoor antenna and cable are additional hardware. Final accessory options will be listed at launch.' },
  { title: 'Dual', heading: 'Work with both indoor antennas.', text: 'Use the built-in antenna together with a separate approved indoor antenna. Positioning and antenna separation still determine how the system works in your space.', image: 'standing', note: 'Dual mode does not guarantee double the coverage. The outdoor signal and room layout still matter.' },
]
...
export default function ProductPage() {
  ...
  const photos = GALLERY.map(([file, , alt]) => ({ src: `${BASE}product/${file}.webp`, alt: `${alt}. Product rendering.` }))
  return <>
    <section className="product-page-hero" id="top"><div className="container">
      ...
      </div><div className="product-detail-copy"><p className="eyebrow">PLIDEPLI cellular signal booster</p><h1>A stronger connection.<br />A simpler setup.</h1><p className="product-lede">Bring your existing outdoor cell signal inside, with the indoor antenna built right into the booster.</p><div className="product-price"><strong>~$275</strong><span>Target campaign price<br />Final pricing at launch</span></div><a className="button button-dark" href="#cta">Notify me at launch<Icon name="arrow" /></a><p className="product-free">Coming to Kickstarter. Join free. No payment today.</p><ul className="product-points"><li><Icon name="check" />Indoor antenna built in</li><li><Icon name="check" />Five supported cellular bands</li><li><Icon name="check" />FCC Part 20 certified</li><li><Icon name="check" />Fixed home and cabin installations</li></ul><a className="text-link" href={`${BASE}#fcc`}>See the authorization and specifications<Icon name="arrow" /></a></div></div>
    </div></section>
    ...
    ].map(([file, title, description]) => <article key={file}><div><img src={`${BASE}product/${file}.webp`} alt={`${title} product rendering`} width="1254" height="1254" loading="lazy" /></div><h3>{title}</h3><p>{description}</p></article>)}</div><div className="kit-cable-note"><Icon name="plus" /><p>The outdoor antenna cable is also shown in the kit image. Cable lengths, fixings, and optional indoor antennas will be specified at launch.</p></div></div></section>
    ...
    <section className="page-crosslink"><div className="container"><div><p className="eyebrow">Meet the team behind it</p><h2>There’s a story<br />inside this hardware.</h2></div><p>Follow the prototypes, the tuning, and the decisions that brought PLIDEPLI to this point.</p><a className="button button-dark" href={`${BASE}stories/`}>Read our full story<Icon name="arrow" /></a></div></section>
